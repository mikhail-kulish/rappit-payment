...
public class JdbcGetMerchantPayments implements BiFunction<Long, Pageable, Page<Payment>> {
    private final JdbcTemplate jdbcTemplate;

    public JdbcGetMerchantPayments(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    @Override
    public Page<Payment> apply(Long merchantId, Pageable pageable) {
        ...
        final Collection<Payment> payments = jdbcTemplate.query(
                "SELECT p.id as pmnt_id, " +
                        ...
                        "FROM payment p LEFT JOIN `order` o ON o.id = p.order_id WHERE o.merchant_id = ? ORDER BY p.id DESC LIMIT ?, ?",
